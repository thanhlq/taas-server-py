"""
The very first code that runs when the outbox_worker starts.
...
``ews_worker.bootstrap`` so the workers share the same configuration model.
"""
...
def setup_environment(env_file: str = '.env') -> tuple[Settings, str]:
    ...
    LogFactory().logger.info('Outbox worker environment setup complete.')
