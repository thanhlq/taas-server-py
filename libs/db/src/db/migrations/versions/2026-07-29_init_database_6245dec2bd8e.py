...
def schema_upgrades() -> None:
    ...
    bind = op.get_bind()
    # Declare the shared enum types with create_type=False so op.create_table()
    # does NOT try to auto-create them (which duplicates the type and errors when
    # a second table reuses it). Create each once, idempotently, up front.
    ...
    sagastatus_enum = postgresql.ENUM('RUNNING', 'COMPLETED', 'COMPENSATING', 'ABORTED', 'FAILED', name='sagastatus', create_type=False)
    schedulejobkind_enum = postgresql.ENUM('ONCE', 'INTERVAL', 'CRON', name='schedulejobkind', create_type=False)
    schedulejobstatus_enum = postgresql.ENUM('SCHEDULED', 'RUNNING', 'DONE', 'FAILED', 'CANCELLED', name='schedulejobstatus', create_type=False)
    ...
    sagastatus_enum.create(bind, checkfirst=True)
    schedulejobkind_enum.create(bind, checkfirst=True)
