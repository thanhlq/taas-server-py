...
class UserOAuthAccount(UUIDv7AuditBase):
    ...
    access_token: Mapped[str] = mapped_column(
        EncryptedText(key=settings.app.SECRET_KEY), nullable=False
    )
    expires_at: Mapped[int | None] = mapped_column(Integer, nullable=True)
    refresh_token: Mapped[str | None] = mapped_column(
        EncryptedText(key=settings.app.SECRET_KEY), nullable=True
    )
    ...
    provider_user_data: Mapped[dict[str, Any] | None] = mapped_column(
        JSON, nullable=True
    )
    ...
    user: Mapped[User] = relationship(
        back_populates='oauth_accounts', viewonly=True, innerjoin=True, lazy='joined'
    )
