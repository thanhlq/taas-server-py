...
from foundation.utils.env_utils import UnsetType, get_env
from foundation.utils.module_loader import module_to_os_path

if TYPE_CHECKING:
    from sqlalchemy.ext.asyncio import AsyncEngine

    from foundation.db.sa_config import SQLAlchemyAsyncConfig
...
"""
Important notes:
    - In this class the bool, int, float,.. types should have type hints, otherwise the get_env will return str type and it will cause
        issues when using the settings.
"""
...
class DatabaseSettings:
    ...
    ECHO: bool = _env_field('DATABASE_ECHO', False, bool)
    """Enable SQLAlchemy engine logs."""
    DEBUG: bool = _env_field('DATABASE_DEBUG', False, bool)

    ECHO_POOL: bool = _env_field('DATABASE_ECHO_POOL', False, bool)
    """Enable SQLAlchemy connection pool logs."""
    POOL_DISABLED: bool = _env_field('DATABASE_POOL_DISABLED', False, bool)
    """Disable SQLAlchemy pool configuration."""
    POOL_SIZE: int = _env_field('DATABASE_POOL_SIZE', 5, int)
    """Pool size for SQLAlchemy connection pool"""
    POOL_MAX_OVERFLOW: int = _env_field('DATABASE_MAX_POOL_OVERFLOW', 30, int)
    """Max overflow for SQLAlchemy connection pool"""
    POOL_TIMEOUT: int = _env_field('DATABASE_POOL_TIMEOUT', 30, int)
    """Time in seconds for timing connections out of the connection pool."""
    POOL_RECYCLE: int = _env_field('DATABASE_POOL_RECYCLE', 300, int)
    # 1800: 30 minutes, 300: 5 minutes
    """
    Recycle below any LB/network idle timeout
    Amount of time to wait before recycling connections.
    """
    POOL_PRE_PING: bool = _env_field('DATABASE_PRE_POOL_PING', False, bool)
    """
