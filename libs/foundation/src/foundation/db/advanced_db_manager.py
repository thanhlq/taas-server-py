...
class AdvancedDBManager:
    ...
    def __init__(self, db_settings: DatabaseSettings | None = None):
        ...
        logger.info(f'Initializing AdvancedDBManager with settings: {db_settings}')
        self._engine = EngineFactory.get_sqlalchemy_engine(db_settings)
        ...
        self.session_stats = DBSessionStats(logger, debug=self.debug)
        """
...
class ConcurrentSessionFactory:
    ...
    # Class variable to hold the session statistics, shared across all instances of ConcurrentSessionFactory
    scoped_session_stats: DBSessionStats | None= None
    ...
    async def commit_sessions(sessions: Iterable[AsyncSession]):
        # sessions_list = list(sessions)
        ConcurrentSessionFactory.scoped_session_stats.increment_committed(len(sessions)) # type: ignore
