...
from foundation.email.transports.base import HTTPResponse, HTTPTransport

if TYPE_CHECKING:
    from foundation.email.transports.aiohttp import AiohttpResponse, AiohttpTransport
    from foundation.email.transports.httpx import HttpxResponse, HttpxTransport
...
def get_transport(transport: str | type["HTTPTransport"] = "httpx") -> "HTTPTransport":
    ...
    if isinstance(transport, str):
        if transport == "httpx":
            from foundation.email.transports.httpx import HttpxTransport

            return HttpxTransport()

        if transport == "aiohttp":
            from foundation.email.transports.aiohttp import AiohttpTransport
...
def __getattr__(name: str) -> object:
    ...
    if name == "HttpxResponse":
        from foundation.email.transports.httpx import HttpxResponse
    ...
    if name == "AiohttpResponse":
        from foundation.email.transports.aiohttp import AiohttpResponse
