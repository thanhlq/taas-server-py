...
- `@websocket(path, name=..., **extra)` decorator in
  `foundation.http.decorator` attaches a `WebSocketRoute` to the handler.
- `BaseController.get_websocket_routes()` collects them (MRO walk, handlers
  bound to the instance), alongside `get_routes()` for HTTP.
- Handlers receive a `WebSocketSession` (`foundation.http._websocket`) — a
...
- Flows through the `foundation.http` controller/adapter abstraction.
