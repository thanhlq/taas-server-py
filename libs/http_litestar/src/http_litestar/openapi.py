...
def build_swagger_plugin(**kwargs: Any) -> SwaggerRenderPlugin:
    """Swagger-UI plugin pre-configured with the eWorkSuite OAuth2/PKCE setup."""
    kwargs.setdefault('init_oauth', SWAGGER_INIT_OAUTH)
    kwargs.setdefault('oauth2_redirect_url', SWAGGER_OAUTH2_REDIRECT_URL)
    return SwaggerRenderPlugin(**kwargs)
...
def build_openapi_config(
    ...
    **kwargs: Any,
    ...
    kwargs.setdefault(
        'render_plugins',
        [build_swagger_plugin(), RedocRenderPlugin(), JsonRenderPlugin()],
    )
