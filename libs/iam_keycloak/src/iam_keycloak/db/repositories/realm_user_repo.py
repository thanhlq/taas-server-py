...
from core.iam.domain.entities import User as User
...
# @instrument
class RealmUserRepository(KeycloakBaseRepository[KeycloakUserEntityOrm, User]):  # type: ignore
    realm_name = None
    realm_id = None
    ...
    def _filter(self, query, **kwargs):
        ...
        if self.realm_id:
            query = query.join(KeycloakUserEntityOrm.realm).where(
                KeycloakRealmOrm.id == self.realm_id
            )
            ...
            raise ValueError(
                'RealmUserRepository: realm_id or realm_name must be set before filtering users.'
            )
    ...
    def from_orm(self, obj: KeycloakUserEntityOrm, **kwargs) -> User:
        ...
        userDefAttributes = User.__annotations__
        ...
        for attribute in obj.attributes:
            ...
            if (
                User.__annotations__.get(name) == Optional[bool]
                or User.__annotations__.get(name) is bool
            ):
