...
from platform_core.types import Empty
from platform_core.utils.predicates import is_dataclass_instance

if TYPE_CHECKING:
    from typing import AbstractSet, Any, Iterable

    from platform_core.types.protocols import DataclassProtocol
...
def extract_dataclass_fields(
    ...
    if exclude_none:
        dataclass_fields = (
            field for field in dataclass_fields if getattr(dt, field.name) is not None
        )
    if exclude_empty:
        dataclass_fields = (
            field for field in dataclass_fields if getattr(dt, field.name) is not Empty
        )
    if include:
        dataclass_fields = (
            field for field in dataclass_fields if field.name in include
        )
    if exclude:
        dataclass_fields = (
            field for field in dataclass_fields if field.name not in exclude
        )
...
def extract_dataclass_items(
    ...
    dataclass_fields = extract_dataclass_fields(
        dt, exclude_none, exclude_empty, include, exclude
    )
...
def simple_asdict(
    ...
    """
    ret = {}
    for field in extract_dataclass_fields(
        obj, exclude_none, exclude_empty, exclude=exclude
    ):
