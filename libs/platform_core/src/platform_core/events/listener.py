...
from platform_core.utils.predicates import is_async_callable
from platform_core.utils.sync import ensure_async_callable

if TYPE_CHECKING:
    from platform_core.types import AnyCallable, AsyncAnyCallable
    ...
    def __call__(self, fn: AnyCallable) -> EventListener:
        ...
        if not callable(fn):
            raise ImproperlyConfiguredException(
                'EventListener instance should be called as a decorator on a callable'
            )
    ...
    def wrap_in_error_handler(fn: AsyncAnyCallable) -> AsyncAnyCallable:
        ...
        async def wrapped(*args: Any, **kwargs: Any) -> None:
            ...
            try:
                ...
                logger.exception(
                    'Error while executing listener %s: %s', fn.__name__, exc
                )
